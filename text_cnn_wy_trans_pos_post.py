...
class TextCNN:
    def __init__(self, sequence_length, num_classes,pos_vocab_size,pos_embedding_size,postion_vocab_size,postion_embedding_size,
        ...
        with tf.variable_scope("position-embeddings"):
            self.W_pos = tf.get_variable("W_postion", [postion_vocab_size, postion_embedding_size], initializer=initializer())
        ...
        self.embedded_chars_expanded = tf.concat(
            [self.embedded_chars, self.pos_embedded_chars, self.p1_embedded_chars,
             self.p2_embedded_chars], -1)
        # self-attention
        with tf.variable_scope("self-attention"):
            self.self_attn_output, self.self_alphas = multihead_attention(self.embedded_chars_expanded, self.embedded_chars_expanded,
                                                                   num_units=792, num_heads=num_heads)
