...
class TextCNN:
    def __init__(self, sequence_length, num_classes,
        ...
        #     self.output_layer = transformer_model(self.embedded_chars) #[?, 100, 768]

        # # self-attention
        # with tf.variable_scope("self-attention"):
        #     self.self_attn_output, self.self_alphas = multihead_attention(self.embedded_chars, self.embedded_chars,
        #                                                                   num_units=512, num_heads=num_heads)
        #     # print("attention shape:", self.self_attn.get_shape) #(?, 90 ,300)
        self.self_attn = tf.layers.dense(self.embedded_chars, units=300, kernel_initializer=initializer())  # [800 90 300]
