...
def multihead_attention(queries, keys, num_units, num_heads,
                        dropout_rate=0.1, scope="multihead_attention", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Residual connection
        outputs += queries
        print("outputs shape 1:", outputs)
        # Normalize
        outputs = layer_norm(outputs)  # (N, T_q, C)
        print("ouput shape 2:",outputs)
...
def layer_norm_2(input_tensor, name=None):
...
def gelu(x):
  ...
  return x * cdf
def fnn(input_tensor, intermediate_size=3072,hidden_size=768,reuse=None,initializer_range=0.02):
    ...
    with tf.variable_scope("ffn_output",reuse=reuse):
        layer_output = tf.layers.dense(
            intermediate_output,
            hidden_size,
            kernel_initializer=create_initializer(initializer_range))
        # layer_output = dropout(layer_output, hidden_dropout_prob)
        layer_output = layer_norm_2(layer_output + input_tensor)
        print("layer_output:", layer_output.get_shape())
    return layer_output

def  attention_entity(inputs,e1, e2):
    # inputs = (batch,  seq_len,hidden)
    # e = (batch,1)
    attention_size=50
    with tf.variable_scope("attention_entity"):
    #     def extract_entity(x, e):
    #         e_idx = tf.concat([tf.expand_dims(tf.range(tf.shape(e)[0]), axis=-1), tf.expand_dims(e, axis=-1)], axis=-1)
    #         return tf.gather_nd(x, e_idx)  # (batch, hidden)

        seq_len = tf.shape(inputs)[1]  # fixed at run-time #90
        hidden_size = inputs.shape[2].value
        # inputs = tf.layers.dense(inputs, units=128, acti)
        # Latent Relation Variable based on Entities
        e1_h = e1  # (batch, hidden)
        e2_h = e2  # (batch, hidden)
        e1_h = tf.reshape(tf.tile(e1_h, [1, seq_len]), [-1, seq_len, hidden_size]) #(batch, seq_len, hidden_size)
        e2_h = tf.reshape(tf.tile(e2_h, [1, seq_len]), [-1, seq_len, hidden_size])  # (batch, seq_len, hidden_size)
        #concat word embedding and entity embedding

        input_e1 = tf.concat([inputs, e1_h], axis=-1)
        input_e2 = tf.concat([inputs, e2_h], axis=-1)
        # input_e1 =  tf.add(inputs, e1_h)
        # input_e2 =  tf.add(inputs, e2_h)
        # print("input_e1:", input_e2.get_shape())
        # e_h = tf.layers.dense(tf.concat([e1_h, e2_h], -1), hidden_size, use_bias=False, kernel_initializer=initializer())
        # e_h = tf.reshape(tf.tile(e_h, [1, seq_len]), [-1, seq_len, hidden_size])
        # v = tf.tanh(tf.add(inputs, e_h))
        #MLP
        # layer1_output = tf.layers.dense(input_e1, units=hidden_size,activation=tf.nn.relu, use_bias=True,kernel_initializer=initializer())
        # layer2_output = tf.layers.dense(input_e2, units=hidden_size, activation=tf.nn.relu, use_bias=True,
        #                                 kernel_initializer=initializer())
        # layer2_output = tf.layers.dense(layer1_output, units=hidden_size, use_bias=True, initializer=tf.keras.initializers.glorot_normal())

        #  For each of the timestamps its vector of size A from `v` is reduced with `u` vector
        u1 = tf.get_variable("u1_var", [hidden_size], initializer=tf.keras.initializers.glorot_normal())
        u2 = tf.get_variable("u2_var", [hidden_size], initializer=tf.keras.initializers.glorot_normal())
        vu_1 = tf.tensordot(input_e1, u1, axes=1, name='vu_1')  # (B,T) shape
        vu_2 = tf.tensordot(input_e2, u2, axes=1, name='vu_2')  # (B,T) shape
        #softmax
        alphas_1 = tf.nn.softmax(vu_1, name='alphas_1')  # (B,T) shape
        alphas_2 = tf.nn.softmax(vu_2, name='alphas_2')  # (B,T) shape
        # alphas = (alphas_1+alphas_2)/2.0
        #output
        output_1 = tf.reduce_sum(inputs * tf.expand_dims(alphas_1, -1), 1)
        output_2 = tf.reduce_sum(inputs * tf.expand_dims(alphas_2, -1), 1)
    return output_1, output_2

# this function just to obtain entity aware weight
def attention_entity_aware(inputs, e1, e2):
    # inputs = (batch,  seq_len,hidden)
    # e = (batch,1)
    with tf.variable_scope("attention_entity_aware"):
        def extract_entity(x, e):
            e_idx = tf.concat([tf.expand_dims(tf.range(tf.shape(e)[0]), axis=-1), tf.expand_dims(e, axis=-1)], axis=-1)
            return tf.gather_nd(x, e_idx)  # (batch, hidden)
        ...
        # e1_h = tf.reshape(tf.tile(e1_h, [1, seq_len]), [-1, seq_len, hidden_size])  # (batch, seq_len, hidden_size)
        # e2_h = tf.reshape(tf.tile(e2_h, [1, seq_len]), [-1, seq_len, hidden_size])  # (batch, seq_len, hidden_size)
        e1_h = tf.expand_dims(e1_h, axis=-1) #[batch, size,1]
        e2_h = tf.expand_dims(e2_h, axis=-1)
        ...
        # u1 = tf.get_variable("u1_var", [hidden_size], initializer=tf.keras.initializers.glorot_normal())
        # u2 = tf.get_variable("u2_var", [hidden_size], initializer=tf.keras.initializers.glorot_normal())
        vu_1 = tf.nn.softmax(tf.reshape(tf.matmul(inputs, e1_h), [batch_size, seq_len])) # (B,T) shape
        vu_2 = tf.nn.softmax(tf.reshape(tf.matmul(inputs, e2_h), [batch_size, seq_len])) # (B,T) shape
        # variant_main
        in_w = (vu_1+vu_2)/2.0
        in_w = tf.reshape(in_w, [batch_size, seq_len, 1])
        # output
        output_1 = tf.multiply(inputs, in_w)

    return output_1

def attention_output(inputs):
