...
class TextCNN:
    def __init__(self, sequence_length, num_classes,
                  text_embedding_size,filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        with tf.device('/device:GPU:0'), tf.variable_scope("text-embedding"):
            ...
            self.text_embedded_chars_trans = transformer.transformerencoder(self.text_embedded_chars_change)
            self.text_embedded_chars_expanded = tf.expand_dims(self.text_embedded_chars_trans, -1) #[800 90 300 1]
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.variable_scope("conv-maxpool-%s" % filter_size):
                # Convolution Layer
                conv = tf.layers.conv2d(self.text_embedded_chars_expanded, num_filters, [filter_size, _embedding_size],
