...
from transformer import multihead_attention, feedforward, attention,attention_4
...
class TextCNN:
    def __init__(self, sequence_length, num_classes,pos_vocab_size,pos_embedding_size,
        ...
        # self.e1_embeded_2 = tf.reshape(tf.tile(self.e1_embeded, [1, sequence_length]), [-1, sequence_length, 768])
        # self.e2_embeded_2 = tf.reshape(tf.tile(self.e2_embeded, [1, sequence_length]), [-1, sequence_length, 768])
        ...
        # self.text_embedded_chars1 = tf.add(self.text_embedded_chars,self.e1_embeded_2)
        # self.text_embedded_chars_2 = tf.add(self.text_embedded_chars1,self.e2_embeded_2)
        ...
        with tf.variable_scope('dropout-embeddings'):
            ...
            # self.e1_embeded = tf.nn.dropout(self.e1_embeded, self.emb_dropout_keep_prob)
            # self.e2_embeded = tf.nn.dropout(self.e2_embeded, self.emb_dropout_keep_prob)
        ...
        # self.text_embedded_chars_2 = tf.concat([self.enc, self.e1_embeded_2, self.e2_embeded_2], axis=1)
        # self.e1_embeded_dense = tf.layers.dense(self.e1_embeded, units=128, activation=tf.nn.tanh,
        #                                         kernel_initializer=initializer())
        # self.e2_embeded_dense = tf.layers.dense(self.e2_embeded, units=128, activation=tf.nn.tanh,
        #                                         kernel_initializer=initializer())
        # self.embeded_dense = tf.layers.dense(self.enc, units=128, activation=tf.nn.tanh,
        #                                      kernel_initializer=initializer())
        # # e1_output,e2_output = attention_entity(self.embeded_dense, self.e1_embeded_dense, self.e2_embeded_dense)
        output = attention_4( self.enc, self.e1_embeded, self.e2_embeded)
        output_dense = tf.layers.dense(output,units=128, activation=tf.nn.tanh)
        ...
        # self.self_atten_change = tf.expand_dims(self.enc_attention, -1)
