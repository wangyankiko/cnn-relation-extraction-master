...
def eval():
    with tf.device('/cpu:0'):
        x_text, y, pos1, pos2 ,x_text_clean, sentence_id = data_helpers.load_data_and_labels(FLAGS.test_path)
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            input_text= graph.get_operation_by_name("text_embedded_chars").outputs[0]
            ...
            batches = data_helpers.batch_iter(list(zip(id)), FLAGS.batch_size, 1, shuffle=False)
            ...
            for batch in batches:
                ...
                # print(a[0])
                # print(list(a[0]))
                text_embedded_chars_dev = server_bert.load_clean_vector("embedding_unclean_test.npy", list(a[0]), sentence_id)
                # print(text_embedded_chars_dev.shape)
                pred = sess.run(predictions, {input_text: text_embedded_chars_dev,
                                              dropout_keep_prob: 1.0,emb_dropout_keep_prob:1.0 })
                preds.append(pred)
