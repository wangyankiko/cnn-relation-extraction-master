...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y,train_text_pos, train_e1, train_e2, train_pos1, train_pos2,train_sentence_len,train_y_label  = data_helper_4.load_data_and_labels(FLAGS.train_path)
    with tf.device('/cpu:0'):
        test_text, test_y, test_text_pos, test_e1, test_e2, test_pos1, test_pos2, test_sentence_len,test_y_label = data_helper_4.load_data_and_labels(FLAGS.test_path)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            batches = data_helper_4.batch_iter(list(zip(train_text, train_y, train_x_pos,train_y_label)),
            ...
            for batch in batches:
                train_bx,  train_by, train_pos,train_biny = zip(*batch)
                # print(x_batch)
                # print(list(x_batch))
                ...
                if step % FLAGS.evaluate_every == 0:
                    print("\nEvaluation:")
                    # Generate batches
                    test_batches = data_helper_4.batch_iter(list(zip(test_text, test_y, test_x_pos,test_y_label)),
                    ...
                    for test_batch in test_batches:
                        test_bx, test_by,test_pos,test_biny = zip(*test_batch)
                        # a = list(test_bx)
                        # print(a)
