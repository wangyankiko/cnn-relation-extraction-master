...
from text_cnn_entity_wy_3 import TextCNN
import datahelper3
import utils
...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y,train_text_pos, train_e1_start, train_e1_end, train_e2_start, train_e2_end, train_sentence_len = datahelper3.load_data_and_labels(FLAGS.train_path)
    with tf.device('/cpu:0'):
        test_text, test_y, test_text_pos, test_e1_start, test_e1_end, test_e2_start,test_e2_end, test_sentence_len = datahelper3.load_data_and_labels(FLAGS.test_path)
    print("e2_start:", train_e2_start)
    print("e2_end:",train_e2_end)
    # Build vocabulary
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            batches = datahelper3.batch_iter(list(zip(train_text, train_y,train_e1_start,train_e1_end, train_e2_start,train_e2_end)),
            ...
            for batch in batches:
                train_bx,  train_by, train_be1_start, train_be1_end, train_be2_start, train_be2_end= zip(*batch)
                # print(x_batch)
                # print(list(x_batch))
                ...
                # print(np.array(train_be1_start).shape)
                # print(train_be1_start)
                # print(train_be1_end)
                # print("shape",text_embedded_chars.shape)
                # e1_embedd = text_embedded_chars[0][train_be1_end[0]]
                # # print(e1_embedd.shape)
                # e2_embedd = text_embedded_chars[0][train_be2_end[0]]
                # for i in range(1, text_embedded_chars.shape[0]):
                #     e1 = text_embedded_chars[i][train_be1_end[i]]
                #     # print("e1 shape",e1.shape)
                #     e2 = text_embedded_chars[i][train_be2_end[i]]
                #     e1_embedd = np.vstack((e1_embedd, e1))
                #     e2_embedd = np.vstack((e2_embedd, e2))
                # e1_index = [ i * FLAGS.max_sentence_length + train_be1_start[i] for i in range(0, text_embedded_chars.shape[0])]
                # e2_index = [i * FLAGS.max_sentence_length + train_be2_start[i] for i in range(0, text_embedded_chars.shape[0])]
                # e1_embedd_change = tf.expand_dims(e1_embedd, axis=1)
                # e2_embedd_change = tf.expand_dims(e2_embedd, axis=1)
                # print(e1_embedd.shape)
                # print(np.array(e1_index).shape)
                # print(np.array(train_be1_start).shape)
                # print(e1_index)
                # print(train_be1_start)
                batch_size_len =int(text_embedded_chars.shape[0])
                # print(batch_size_len)
                FLAGS.batch_size_len = batch_size_len
                feed_dict = {
                    cnn.text_embedded_chars: text_embedded_chars,
                    cnn.input_y: train_by,
                    cnn.e1_start:train_be1_start,
                    cnn.e2_start:train_be2_start,
                    cnn.e1_end: train_be1_end,
                    cnn.e2_end:train_be2_end,
                    cnn.batch_size_len: batch_size_len,
                ...
                if step % FLAGS.evaluate_every == 0:
                    print("\nEvaluation:")
                    # Generate batches
                    test_batches = datahelper3.batch_iter(list(zip(test_text, test_y,test_e1_start, test_e1_end, test_e2_start,test_e2_end)),
                    ...
                    for test_batch in test_batches:
                        test_bx, test_by,test_be1_start,test_be1_end, test_be2_start,test_be2_end = zip(*test_batch)
                        # a = list(test_bx)
                        # print(a)
                        ...
                        # test_e1_embedd = text_embedded_chars[0][test_be1_end[0]]
                        # # print(e1_embedd.shape)
                        # test_e2_embedd = test_text_embedded_chars[0][test_be2_end[0]]
                        # for i in range(1, test_text_embedded_chars.shape[0]):
                        #     test_e1 = test_text_embedded_chars[i][test_be1_end[i]]
                        #     # print("e1 shape", test_e1.shape)
                        #     test_e2 = test_text_embedded_chars[i][test_be2_end[i]]
                        #     test_e1_embedd = np.vstack((test_e1_embedd, test_e1))
                        #     test_e2_embedd = np.vstack((test_e2_embedd, test_e2))
                        # e1_index = [i * FLAGS.max_sentence_length + test_be1_start[i] for i in
                        #             range(0, test_text_embedded_chars.shape[0])]
                        # e2_index = [i * FLAGS.max_sentence_length + test_be2_start[i] for i in
                        #             range(0, test_text_embedded_chars.shape[0])]
                        # print(np.array(e1_index).shape)
                        # print(np.array(test_be1_start).shape)
                        # print(e1_index)
                        # print(e2_index)
                        test_batch_size_len = int(test_text_embedded_chars.shape[0])
                        FLAGS.batch_size_len = test_batch_size_len
                        # print(FLAGS.batch_size_len)
                        feed_dict = {
                            cnn.text_embedded_chars: test_text_embedded_chars,
                            cnn.input_y: test_by,
                            cnn.e1_end: test_be1_end,
                            cnn.e1_start:test_be1_start,
                            cnn.e2_start:test_be2_start,
                            cnn.e2_end: test_be2_end,
                            cnn.batch_size_len: FLAGS.batch_size_len,
