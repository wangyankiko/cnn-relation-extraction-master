    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if FLAGS.embedding_path:
                pretrain_W = utils.load_word2vec(FLAGS.embedding_path, FLAGS.text_embedding_size, text_vocab_processor)
            ...
            for batch in batches:
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    summaries, loss, accuracy, predictions,text_expand_shape, pos_shape,embedding_size_shape ,embedding_shape, text_shape= sess.run(
                        [dev_summary_op, cnn.loss, cnn.accuracy, cnn.predictions,cnn.text_expand_shape,cnn.pos_expand_shape,cnn.embedding_size_shape,cnn.embedd_shape,cnn.text_shape], feed_dict)
                    dev_summary_writer.add_summary(summaries, step)

                    time_str = datetime.datetime.now().isoformat()
                    f1 = f1_score(np.argmax(y_dev, axis=1), predictions, labels=np.array(range(1, 19)), average="macro")
                    precision = tf.metrics.precision(np.argmax(y_dev, axis=1), predictions,weights=1)
                    recall = tf.metrics.recall(np.argmax(y_dev, axis=1), predictions,weights=1)
