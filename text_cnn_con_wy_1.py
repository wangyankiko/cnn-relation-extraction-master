...
from transformer import multihead_attention, feedforward, attention_pool
...
class TextCNN:
    def __init__(self, sequence_length, num_classes,pos_vocab_size,pos_embedding_size,
        ...
        self.in_y = tf.placeholder(dtype=tf.int32, shape=[None], name='in_y')  # relations
        ...
        #     self.text_embedd = bc.encode(self.input_text.eval())
        #     # print("text_shape:", self.text_embedd.shape)
        ...
        rel_embed = tf.get_variable(initializer=initializer, shape=[19, num_filters], name='relation_embed')
        self.embedding_y = tf.nn.embedding_lookup(rel_embed, self.in_y, name='y')  # bz, dc
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.variable_scope("conv-maxpool-%s" % filter_size):
                # Convolution Layer
                # w = tf.get_variable(initializer=initializer(), shape=[i, _embedding_size, 1, num_filters], name='weight')
                # b = tf.get_variable(initializer=initializer(), shape=[num_filters], name='bias')
                # conv = tf.nn.conv2d(self.self_atten_change, w, strides=[1, 1, _embedding_size, 1], padding="SAME")  # bz, n, 1, dc
                ...
                conv = tf.layers.conv2d(self.self_atten_change, num_filters, [filter_size, _embedding_size],
                                        kernel_initializer=initializer(), activation=tf.nn.relu,padding="SAME",strides=(1,_embedding_size),
