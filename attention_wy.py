...
def attention(inputs,input_cnn, e1, e2, p1, p2, attention_size):
    ...
    # input_cnn process
    input_cnn_dense = tf.layers.dense(input_cnn, units=latent_size, use_bias=False, kernel_initializer=initializer)
    # output
    output = tf.reduce_sum(input_cnn_dense * tf.expand_dims(alphas, -1), 1)  # (batch, hidden)
...
def multihead_attention(queries, keys, num_units, num_heads,
                        dropout_rate=0.3, scope="multihead_attention", reuse=None):
