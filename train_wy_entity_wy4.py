    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for batch in batches:
                ...
                # text_embedded_chars_cls = text_embedded_chars[0][0]
                e1_embedd = np.sum(text_embedded_chars[0][train_be1_start[0]:train_be1_end[0]+1],axis=0)
                # print(e1_embedd.shape)
                e2_embedd = np.sum(text_embedded_chars[0][train_be2_start[0]:train_be2_end[0]+1],axis=0)
                for i in range(1, text_embedded_chars.shape[0]):
                    e1 = np.sum(text_embedded_chars[i][train_be1_start[i]:train_be1_end[i]+1], axis=0)
                    # print("e1 shape",e1.shape)
                    e2 = np.sum(text_embedded_chars[i][train_be2_start[i]:train_be2_end[i]+1], axis=0)
                    text_cls = text_embedded_chars[i][0]
                    # text_embedded_chars_cls = np.vstack((text_embedded_chars_cls, text_cls))
                ...
                feed_dict = {
                    ...
                    cnn.e1_embeded:e1_embedd,
                    cnn.e2_embeded:e2_embedd,
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    for test_batch in test_batches:
                        ...
                        # test_text_embedded_chars_cls = text_embedded_chars[0][0]
                        test_e1_embedd = np.sum(text_embedded_chars[0][test_be1_start[0]:test_be1_end[0]+1],axis=0)
                        # print(e1_embedd.shape)
                        test_e2_embedd = np.sum(test_text_embedded_chars[0][test_be2_start[0]:test_be2_end[0]+1], axis=0)
                        for i in range(1, test_text_embedded_chars.shape[0]):
                            test_e1 = np.sum(test_text_embedded_chars[i][test_be1_start[i]:test_be1_end[i] + 1], axis=0)
                            # print("e1 shape", test_e1.shape)
                            test_e2 = np.sum(test_text_embedded_chars[i][test_be2_start[i]:test_be2_end[i] + 1], axis=0)
                            test_cls = text_embedded_chars[i][0]
                            test_e1_embedd = np.vstack((test_e1_embedd, test_e1))
                            test_e2_embedd = np.vstack((test_e2_embedd, test_e2))
                            # test_text_embedded_chars_cls = np.vstack((test_text_embedded_chars_cls, test_cls))
                        feed_dict = {
                            cnn.text_embedded_chars: test_text_embedded_chars,
                            cnn.input_y:test_by,
                            cnn.e1_embeded:test_e1_embedd,
                            cnn.e2_embeded:test_e2_embedd,
