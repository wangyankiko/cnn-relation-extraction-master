...
class TextCNN:
    def __init__(self, sequence_length, num_classes,pos_vocab_size,pos_embedding_size,
        ...
        #     e1 =
        # tf.while_loop
        self.embedd_dense = tf.layers.dense(self.embedded_chars, units=300, activation=tf.nn.relu, use_bias=True,
                                         trainable=True, kernel_initializer=initializer())
        self.text_embedded_chars_re = tf.reshape(self.embedd_dense, [tf.shape(self.embedd_dense)[0] * tf.shape(self.embedd_dense)[1], 300])
        print(self.text_embedded_chars_re.get_shape())
        self.e1_embedded = tf.nn.embedding_lookup(self.text_embedded_chars_re, self.e1_index)
        e2_embedded = tf.nn.embedding_lookup(self.text_embedded_chars_re, self.e2_index)
        e1_output, e2_output = attention_entity(self.embedd_dense, self.e1_embedded, e2_embedded)
        ...
        self.h_pool_flat_2 = tf.concat([self.h_pool_flat, e1_output,e2_output], axis=-1)
