...
from text_cnn_wy_trans_entity_atten_input import TextCNN
import data_helper2
import utils
...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y,train_text_pos, train_e1, train_e2, train_pos1, train_pos2,train_sentence_len,train_sentence_tokens = data_helper2.load_data_and_labels(FLAGS.train_path)
    with tf.device('/cpu:0'):
        test_text, test_y, test_text_pos, test_e1, test_e2, test_pos1, test_pos2, test_sentence_len,test_sentence_tokens = data_helper2.load_data_and_labels(FLAGS.test_path)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            batches = data_helper2.batch_iter(list(zip(train_y,train_x_pos,train_e1, train_e2,train_text, train_sentence_tokens)),
            ...
            for batch in batches:
                train_by, train_pos,train_be1,train_be2 ,train_bx,train_tokens= zip(*batch)
                # print(x_batch)
                # print(list(x_batch))
                ...
                if step % FLAGS.evaluate_every == 0:
                    print("\nEvaluation:")
                    # Generate batches
                    test_batches = data_helper2.batch_iter(list(zip(test_y,test_x_pos,test_e1, test_e2,test_text,test_sentence_tokens)),
                    ...
                    for test_batch in test_batches:
                        test_by,test_pos,test_be1, test_be2,test_bx,test_tokens = zip(*test_batch)
                        # a = list(test_bx)
                        # print(a)
                        # test_text_embedded_chars = server_bert.load_vector("embedding_unclean_test.npy", list(test_bx))  # [20 90 768)
                        test_text_embedded_chars = bc.encode(list(test_tokens))
                        feed_dict = {
                            cnn.text_embedded_chars: test_text_embedded_chars,
                            cnn.input_y:test_by,
                            cnn.input_e1: test_be1,
                            cnn.input_e2: test_be2,
