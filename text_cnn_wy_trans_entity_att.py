...
from attention import multihead_attention,attention_entity
...
class TextCNN:
    def __init__(self, sequence_length, num_classes,
        ...
        self.self_atten_change =tf.expand_dims(self.self_attn_output, -1) #[800 90 300 1]
        ...
        # entity attention
        self.e1_output, self.e2_output = attention_entity(self.embedded_chars, self.input_e1,self.input_e2)


        # Add dropout
        with tf.variable_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.h_pool_flat, self.dropout_keep_prob)
            self.e1_output_drop = tf.nn.dropout(self.e1_output, self.dropout_keep_prob)
            self.e2_output_drop = tf.nn.dropout(self.e2_output, self.dropout_keep_prob)

        #concat
        self.output_concat = tf.concat([self.h_drop,self.e1_output_drop, self.e2_output_drop], axis=-1)

        #MLP
        # self.output_dense = tf.layers.dense(self.output_concat, units=text_embedding_size, activation=tf.nn.tanh,use_bias=True,
        #                                     kernel_initializer=initializer()
        #                                     )
        # Final scores and predictions
        with tf.variable_scope("output"):
            self.logits = tf.layers.dense(self.output_concat, num_classes, kernel_initializer=initializer())
