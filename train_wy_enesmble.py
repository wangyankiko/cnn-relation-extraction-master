...
from enesmble_cnn_lstm import TextCNN
...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y, train_e1, train_e2, train_pos1, train_pos2, train_sentence_len = data_helper2.load_data_and_labels(FLAGS.train_path)
    with tf.device('/cpu:0'):
        test_text, test_y, test_e1, test_e2, test_pos1, test_pos2, test_sentence_len = data_helper2.load_data_and_labels(FLAGS.test_path)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            train_batches = data_helpers2.batch_iter(list(zip(id_train,train_x, train_y, train_text,
                                                             train_e1, train_e2, train_p1, train_p2)),
                                                    FLAGS.batch_size, FLAGS.num_epochs)
            # Training loop. For each batch...
            best_f1 = 0.0  # For save checkpoint(model)
            for train_batch in train_batches:
                train_bx,train_bx1, train_by, train_btxt, train_be1, train_be2, train_bp1, train_bp2 = zip(*train_batch)
                text_embedded_chars = server_bert.load_clean_vector("/home/wangyan/relaton-extraction/embedding_unclean.npy", list(train_bx), train_sentence_len)  # [20 90 768)
                feed_dict = {
                ...
                if step % FLAGS.evaluate_every == 0:
                    print("\nEvaluation:")
                    # Generate batches
                    test_batches = data_helpers2.batch_iter(list(zip(id_test,test_x, test_y, test_text,
                    ...
                    for test_batch in test_batches:
                        test_bx, test_bx1, test_by, test_btxt, test_be1, test_be2, test_bp1, test_bp2 = zip(*test_batch)
                        test_text_embedded_chars = server_bert.load_clean_vector("/home/wangyan/relaton-extraction/embedding_unclean_test.npy", list(test_bx), test_sentence_len)  # [20 90 768)
                        feed_dict = {
                            model.text_embedded_chars: test_text_embedded_chars,
