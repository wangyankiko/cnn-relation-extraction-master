...
class TextCNN:
    def __init__(self, sequence_length, num_classes,
        ...
        # self.cls_embedding = tf.placeholder(tf.float32, shape=[None, 768], name="cls_embedding")
        ...
        self.self_atten_change = tf.expand_dims(self.layer_output, -1)  # [800 90 300 1]
        ...
        self.cls_embedding = tf.squeeze(self.text_embedded_chars[:, 0:1, :], axis=1)

        #MLP
        with tf.variable_scope("mlp"):
            self.h_drop_flat= tf.layers.dense(self.h_pool_flat, units=300, activation=tf.nn.relu, use_bias=True)
            self.cls_embedding = tf.layers.dense(self.cls_embedding, units=768, activation=tf.nn.relu, use_bias=True)
            self.cls_embedding = tf.layers.dense(self.cls_embedding, units=300, activation=tf.nn.relu, use_bias=True)
        # Add dropout
        with tf.variable_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.h_pool_flat, self.dropout_keep_prob)
            self.cls_embedding_drop = tf.nn.dropout(self.cls_embedding,self.dropout_keep_prob)
        ...
        with tf.variable_scope("output"):
            self.cat_all = tf.concat([self.h_drop, self.cls_embedding_drop], axis=1)
            self.logits = tf.layers.dense(self.cat_all, num_classes, kernel_initializer=initializer())
