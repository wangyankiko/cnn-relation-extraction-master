...
class TextCNN:
    def __init__(self, sequence_length, num_classes,
        ...
        with tf.device('/cpu:0'), tf.variable_scope("text-embedding"):
            ...
            self.text_embedded_chars_expanded = tf.expand_dims(self.text_embedded_chars, -1) #[800 90 300 1]
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.variable_scope("conv-maxpool-%s" % filter_size):
                ...
                conv = tf.layers.conv2d(self.embedded_chars_expanded, num_filters, [filter_size, _embedding_size],
                                        kernel_initializer=initializer(), activation=tf.nn.relu, name="conv") #num_filter=128,filter_size=2,3,4,5
